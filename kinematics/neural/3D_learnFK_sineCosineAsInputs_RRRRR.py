...
class network(object):
	def __init__(self, params):
		# tf Graph input
		self.X = tf.placeholder(shape=[None, params['dof']], dtype=tf.float64)
		self.Y = tf.placeholder(shape=[None, params['numOutput']], dtype=tf.float64)
...
if __name__ == '__main__':
	...
	if(args.mode == 'train'):
		...
		# # h_jointAngles = tf.concat([X, h_jointAngles],1)
		...
		# # layer5 = layers.fully_connected(inputs=layer4, num_outputs=params['hiddenSize'], biases_initializer=tf.zeros_initializer(), activation_fn=tf.nn.relu)
		# outputLayer = layers.fully_connected(inputs=layer2, num_outputs=params['numOutput'], biases_initializer=tf.zeros_initializer(), activation_fn=None)
		...
		with tf.Session() as sess:
			...
			for step in range(1, params['numSteps']+1):
				...
				if step % params['saveStep'] == 0 or step == 1:
					...
					save_path = saver.save(sess, './model/' +  args.robot + '_NN/model-' + str(step) + '.ckpt')
			
			save_path = saver.save(sess, './model/' +  args.robot + '_NN/model-final' + '.ckpt')
	...
	if(args.mode == 'test'):
		...
		with tf.Session() as sess:
			sess.run(init)
			saver = tf.train.import_meta_graph('model/' + args.model + '_NN/model-final.ckpt.meta')
