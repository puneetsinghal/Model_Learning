...
def generateTrajectory(dof, numOutput=2):
	...
	batch_x = batch_x.T
	batch_y = np.zeros((batch_x.shape[0], numOutput))
...
if __name__ == '__main__':
	...
	layer3 = layers.fully_connected(inputs=layer2, num_outputs=128, biases_initializer=tf.zeros_initializer(), activation_fn=tf.nn.relu)
	...
	if(args.mode=='train'):
		...
		with tf.Session() as sess:
			...
			for step in range(1, numSteps+1):
				batch_x, batch_y, batch_l = generateData(batchSize, dof, numOutput)
	...
	with tf.Session() as sess:
		...
		batch_x, batch_y, batch_l = generateTrajectory(dof, numOutput)
