...
if __name__ == '__main__':
	...
	modifiedInputs = tf.concat([tf.sin(inputCombinations), tf.cos(inputCombinations)],1)
	...
	encodedValues = layers.fully_connected(inputs=modifiedInputs, num_outputs=numEncoders, biases_initializer=tf.zeros_initializer(), activation_fn=tf.nn.relu)
	...
	outputs = layers.fully_connected(inputs=encodedValues, num_outputs=numInput, biases_initializer=tf.zeros_initializer(), activation_fn=None)

	# Define loss and optimizer
	errorVector = tf.sqrt(tf.reduce_sum((outputs - modifiedInputs)**2, 1))
	...
	with tf.Session() as sess:
		...
		encValues = sess.run(encodedValues, feed_dict={X: batch_x})#, L:batch_l})
		print(encValues)
		# print(out.shape)
