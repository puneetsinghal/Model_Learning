classdef (Sealed) HebiTrajectoryGenerator
    ...
    %
    % Note that this API is currently in an experimental state. There are
    % still some missing features and the methods call may require changes
    % in future revisions.
    %
    ...
    %      getMinDuration    - gets the minim duration
    ...
    %
    %
    % Note that the trajectory generator requires a defined HebiKinematics
    % setup for determining joint velocity limits and to calculate torques
    % to compensate for accelerations.
    %
    ...
    %      kin = HebiKinematics();
    %      kin.addBody('X5-4');
    %      kin.addBody('X5Link', 'ext', 0.2875, 'twist', 0, 'mass', 0.318);
    %      kin.addBody('X5-1');
    %      kin.addBody('X5Link', 'ext', 0.3678, 'twist', 0, 'mass', 0.276);
    %      kin.addBody('X5-1');
    ...
    methods(Access = public)
        ...
        function this = setAlgorithm(this, varargin)
            ...
            %
            %   'UnconstrainedQp' creates advanced trajectories that
            %   support user specified velocity and acceleration
            %   constraints at any point (free constraints are represented
            %   by nans). The current implementation is however
            %   more expensive to compute and is only recommended
            %   for low numbers of waypoints.
            %
        ...
        function trajectory = newLinearMove(this, positions, varargin)
            ...
            %   an approximate straight line in carthesian workspace
            %   coordinates.
            %
        ...
        function [] = moveJoint(this, group, positions, varargin)
            ...
            %      trajGen.executeTrajectory(traj);
            %
            %   Arguments:
            %
        ...
        function [] = executeTrajectory(this, varargin)
            ...
            %           'DynamicsComp', true, ...
            %           'TorqueOffset', zeros(1, kin.getNumDoF), ...
            %           'Callback', @(time, fbk, cmd) cmd);
            %
            ...
            parser.addParameter('TorqueOffset', [], @(a) isvector(a));
            ...
            while t < duration
                ...
                if p.EnableDynamicsComp
                    accelCompTorque = this.kin.getDynamicCompTorques(...
                        jacobianPosition, ...
                        pos, ...
                        vel, ...
                        accel);
                    torque = torque + accelCompTorque;
                    useTorques = true;
                end
                
                % Account for gravity if applicable
                if ~isempty(p.GravityVec)
                    gravCompTorque = this.kin.getGravCompTorques(...
                        jacobianPosition, ...
                        p.GravityVec);
                    torque = torque + gravCompTorque;
                    useTorques = true;
                end
                
                % Offset for springs etc., if applicable
                if useTorques && ~isempty(p.TorqueOffset)
                    torque = torque + p.TorqueOffset;
                end
                
                % Disable torque control if not needed
                if ~useTorques
                    torque = nan(size(pos));
                end
